...
class TestRunner {
   ...
   */
  private async runSuite(suite: string): Promise<void> {
   ...
   */
  private buildJestConfig(suite: string): string {
    const baseConfig = {
   ...
   * Build Jest command
   */
  private buildJestCommand(suite: string, config: string): string {
   ...
   */
  private getTestMatchPattern(suite: string): string[] {
    switch (suite) {
      case 'unit':
   ...
   */
  private getCoveragePatterns(suite: string): string[] {
    const basePatterns = [
      'src/**/*.ts',
      '!src/**/*.d.ts',
      '!src/**/*.interface.ts',
   ...
   */
  private parseJestOutput(output: string, suite: string): TestResult {
